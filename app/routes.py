...
from .model import APIKey, Event, GPSPosition, Location, User
    ...
    if request.method == "POST":
        ...
        if action == "create":
            ...
            if not key_name:
                ...
                new_key = APIKey(
                    user_id=current_user.id, name=key_name, key=APIKey.generate_key()
                )
...
def timeline():
    ...
    start_of_day_utc = user_tz.localize(
        datetime.combine(current_date, datetime.min.time())
    ).astimezone(pytz.UTC)
    end_of_day_utc = user_tz.localize(
...
def add_event():
    ...
    if lat and lon:
        ...
        if nearby_points:
            ...
            local_time = (
                nearby_points[0].timestamp.replace(tzinfo=pytz.UTC).astimezone(user_tz)
            )
        ...
        form.location_id.choices = [
            ...
            *[
                (str(l.id), l.place_name + f" ({l.distance:0.1f}m)")
    ...
    if len(form.location_id.choices) == 1:
        form.location_id.choices.extend(
            (str(l.id), l.place_name)
            for l in Location.query.filter_by(user_id=current_user.id).all()
        )
    ...
    return render_template("add_event.html", form=form, date=current_date)
    ...
    if request.method == "POST":
        ...
        for file in files:
            ...
            if file and file.filename.endswith(".gpx"):
                ...
                errors.append(
                    f"Invalid file type: {file.filename}. Only .gpx files are allowed."
                )

        if files_processed > 0:
            flash(
                f"Successfully imported {total_points_added} GPS points from {files_processed} file(s).",
                "success",
            )
...
def gps_data():
    ...
    gps_count = (
        GPSPosition.query.filter_by(user_id=current_user.id)
...
def checkin():
    ...
    if lat and lon:
        ...
        try:
            ...
            foursquare_venues = get_foursquare_venues(
                lat, lon, radius=max_distance_km * 1000, limit=50
            )
    ...
    if form.validate_on_submit():
        ...
        if form.location_type.data == "user":
            ...
            event = Event(
                user_id=current_user.id,
                title=form.title.data
                or f"Checked in at {location.place_name}",  # Use custom title if provided
                event_type=form.event_type.data,
            ...
            event = Event(
                user_id=current_user.id,
                title=form.title.data
                or f"Checked in at {place_name}",  # Use custom title if provided
                event_type=form.event_type.data,
                start_time=current_time,
                location_id=new_location.id,
                notes=form.notes.data,
    ...
    return render_template(
        "checkin.html", form=form, locations=combined_locations, lat=lat, lon=lon
    )
...
def get_foursquare_venues(lat, lon, radius=1000, limit=10):
    ...
    try:
        ...
        for venue in data.get("results", []):
            ...
            # Get category
            category = (
                venue.get("categories", [{}])[0].get("name", "Uncategorized")
